...
- Maximize recall to avoid false negatives  
- Interpret results
...
###  Tune KNN model
To make the model as accurate as possible, we need to decide how much observations (neighbors or k) it should use when classifying a new observation. We experiment with different settings of k in a range of 1 to the size of the testing set and compare how well the model performs with each. From the plot, we can see that the number of neighbors that gives us the highest accuracy is `r best_k`.
